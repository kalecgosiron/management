...
var checkSql = require('../db/sql/checkSql')
...
router.post('/checkOrder', function(req, res, next) {
  ...
  if (dateStart != dateEnd) {
    if (ordertype == '车险') {
      var sql =
        "select ordernumber,insured,licenseplate,ordersignature,charge,date_format(date,'%Y-%m-%d') as date,dispatchclerk,orderprintnumber,attributiondepartment from `车险` where"
      var data = []
      if (ordernumber != '') {
        sql += ' ordernumber=?'
      ...
      db.query(sql, data, (err, results) => {
        ...
        if (results == '') {
          ...
          res.send({ code: 404, results: results })
        } else {
      ...
      var sql =
        "select ordernumber,applicant,ordersignature,charge,date_format(date,'%Y-%m-%d') as date,dispatchclerk,orderprintnumber,attributiondepartment from `非车险` where"
      var data = []
      if (ordernumber != '') {
        sql += ' ordernumber=?'
        data.push(ordernumber)
